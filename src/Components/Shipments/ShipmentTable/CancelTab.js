...
//   const CancelData = data.filter((item) => item.status === "Cancelled");
...
//   const endIndex = Math.min(startIndex + itemsPerPage, CancelData?.length);

//   // Extract the data for the current page
//   const currentPageData = CancelData?.slice(startIndex, endIndex);
...
const CancelTab = () => {
  ...
  // const filteredData = filterData(data);
  const startIndex = (currentPage - 1) * itemsPerPage;
  const endIndex = Math.min(startIndex + itemsPerPage, CancelledData?.length);

  // Extract the data for the current page
  const currentPageData = CancelledData?.slice(startIndex, endIndex);
  ...
  return (
    ...
    >
      ...
      <Pagination
                currentPage={currentPage}
                setCurrentPage={setCurrentPage}
                totalItems={CancelledData?.length}
              />
      <Steppertrack
        isModalOpen={isModalOpen}
        handleCancel={handleCancel}
